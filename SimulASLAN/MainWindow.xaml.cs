...
namespace WpfApp1
{
    ...
    {
        private DronePhysics _physics = new DronePhysics();
        ...
        private string _language; // Store language preference
        private const double MapSideMeters = 833;

        // Updated Constructor to accept Language
        public MainWindow(double lat, double lon, int quality, string language)
        {
            ...
            ApplyLanguage(); // Set text based on language
        ...
        private async Task InitializeMapAsync()
        {
            ...
            // Unique filename based on coords
            ...
            if (!File.Exists(finalPath))
            {
                ...
                bool found = false;
                // Wait loop: Check every second for 60 seconds
                for (int i = 0; i < 60; i++)
                {
                    if (File.Exists(sourceFile))
                    {
                        // Found it! Wait a split second to ensure download finished writing to disk
                        ...
                        {
                            // File locked by browser, try next loop iteration
                        }
                    }
        ...
        private void InitializeDroneVisuals()
        {
            ...
            {
                // FIX: Replaced MeshBuilder (causes errors) with BoxVisual3D shapes (safe)
                ...
                // Add directly to the ModelVisual3D children
